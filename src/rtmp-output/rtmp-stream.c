...
static bool rtmp_stream_start(void *data)
{
	...
	/* Beacon: mint a new stream-session id only on a FRESH start. libobs drives
	 * reconnects by re-calling this start with obs_output_reconnecting()==true on
	 * the SAME output object, so we keep the existing id across the reconnect loop
	 * and only regenerate on a user Stop/Start (reconnecting==false). */
	if (!obs_output_reconnecting(stream->output) || stream->beacon_session_id[0] == '\0') {
		char *uuid = os_generate_uuid();
		snprintf(stream->beacon_session_id, sizeof(stream->beacon_session_id), "%s", uuid ? uuid : "");
		bfree(uuid);
		info("Beacon: new stream session id %s", stream->beacon_session_id);
	} else {
		info("Beacon: keeping stream session id %s (reconnect)", stream->beacon_session_id);
	}
